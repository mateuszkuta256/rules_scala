...
def setup_scalafmt_toolchains():
    setup_scalafmt_toolchain(
        ...
        scalafmt_classpath = _SCALAFMT_DEPS,
    )
    for scala_version in SCALA_VERSIONS:
        sanitized_scala_version = sanitize_version(scala_version)
        setup_scalafmt_toolchain(
            name = sanitized_scala_version + "_scalafmt_toolchain",
            scala_version = scala_version,
            scalafmt_classpath = [dep + "_" + sanitized_scala_version for dep in _SCALAFMT_DEPS],
        )
